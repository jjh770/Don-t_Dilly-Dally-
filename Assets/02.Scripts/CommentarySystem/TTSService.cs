...
public class TTSService : MonoBehaviour
{
    ...
    public async Awaitable<AudioClip> GenerateSpeech(string text)
    {
        ...
        {
            Debug.LogError("[TTSService] Google Cloud API 키가 없습니다.");
            return null;
        }

        if (string.IsNullOrEmpty(text))
        {
            Debug.LogWarning("[TTSService] 텍스트가 비어있습니다.");
            return null;
        }
        ...
        try
        {
            ...
            {
                Debug.LogError($"[TTSService] 응답 실패: {request.error}");
                return null;
            }
        ...
        {
            Debug.LogError($"[TTSService] 예외: {e.Message}");
            return null;
        }
    }
    ...
    private AudioClip ParseResponseToAudioClip(string json)
    {
        try
        {
            ...
            {
                Debug.LogError("[TTSService] 음성 데이터가 없음");
                return null;
            }
        ...
        {
            Debug.LogError($"[TTSService] 응답 파싱 실패: {e.Message}");
            return null;
        }
    }
